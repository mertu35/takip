...
const Barcode = ({ text }) => {
  ...
  useEffect(() => {
    ...
    // Yüksek çözünürlüklü ölçekleme (HDPI)
    const scale = 2;
    ...
    // Barkodun altına fiş numarasını yaz
    ctx.fillStyle = "#000000";
...
const Sales = () => {
  ...
  // --- FİNANSAL TOPLAMLAR ---
  const totalBeforeTaxAndDiscount = cart.reduce((sum, item) => sum + item.total, 0);
  // KDV Dahil toplamları ve KDV tutarını hesapla
  // Burada fiyatların KDV hariç olduğunu varsayıyoruz. 
  // Fiş hesaplama:
  const taxAmount = cart.reduce((sum, item) => sum + (item.total * (item.taxRate / 100)), 0);
  const totalAmount = totalBeforeTaxAndDiscount;
  ...
  const handleCreateSaleSubmit = async (e) => {
    ...
    try {
      ...
      // Stokları güncellemek için verileri yeniden çek
      fetchInitialData();
          ...
          <div className="modal-content animate-slide-up" style={{ maxWidth: "480px", backgroundColor: "#fff", color: "#000" }} role="dialog" aria-modal="true">
            ...
            <div className="modal-body print-area" style={{ padding: "2rem", fontSize: "0.85rem", color: "#000", fontFamily: "'Courier New', Courier, monospace" }}>
              ...
              <div style={{ display: "flex", flexDirection: "column", gap: "0.25rem", marginBottom: "1rem" }}>
                ...
                <div><strong>Tarih:</strong> {new Date(lastCreatedSale.createdAt).toLocaleString('tr-TR')}</div>
