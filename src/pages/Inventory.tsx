...
const Inventory = () => {
  ...
  const handleProductSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (modalMode === "add") {
        // payload, productForm (Record<string, any>) üzerine kurulu olduğu için
        // TS'in spread sonucu tam Product şeklini çıkaramaması nedeniyle
        // burada bilinçli olarak `as any` kullanılıyor (bkz. tsconfig.json
        // "kademeli TypeScript geçişi" notu).
        await addProduct(payload as any, user!.uid, user!.displayName, user!.role);
        showToast("Ürün başarıyla eklendi.", "success");
      } else {
        await updateProduct(selectedProductId, payload as any, user!.uid, user!.displayName, user!.role);
  ...
  const handleConfirmImport = async () => {
    ...
    for (const row of importRows) {
      try {
        ...
        const categoryName = cat?.name || row.categoryName;
          ...
          <table className="table">
            <thead>
              <tr>
                ...
                <th onClick={() => handleSort("categoryName" as keyof Product)} style={{ cursor: "pointer" }}>
                  Kategori {sortField === "categoryName" ? (sortOrder === "asc" ? " ▲" : " ▼") : ""}
                </th>
                <th onClick={() => handleSort("costPrice" as keyof Product)} style={{ textAlign: "right", cursor: "pointer" }}>
                ...
                <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: "1rem" }}>
                  <div className="form-group">
                    ...
                    <input
                      ...
                      placeholder="0.00"
                    />
                    <div style={{ fontSize: "0.75rem", color: "var(--text-muted)", marginTop: "0.2rem" }}>Alış / üretim maliyeti</div>
                  </div>
                  <div className="form-group">
                    ...
                    {parseFloat(productForm.costPrice) > 0 && parseFloat(productForm.price) > 0 && (
                      <div style={{ fontSize: "0.75rem", marginTop: "0.2rem", color: "var(--success)", fontWeight: 600 }}>
